...
import type {
  AccountTypePoint,
  BudgetAdherence,
  SubscriptionsLoansSummary,
  TransactionPoint,
} from "@/features/analytics/finance/contracts";
import type { OverviewSummary } from "@/features/analytics/overview/contracts";
import {
  formatCurrency,
  formatInteger,
  formatNumber,
...
export function FinancePage() {
  ...
  const subscriptionsLoans = useApiData<SubscriptionsLoansSummary>(
  ...
  const accountTypes = useApiData<AccountTypePoint[]>(
      ...
      <div className="flex items-start gap-3 rounded-lg border bg-muted/35 p-4">
        ...
        <div>
          ...
          <p className="mt-1 text-xs leading-5 text-muted-foreground">
            Values on this page cannot be traced to an individual user, account,
            or transaction.
          </p>
        </div>
      </div>
      ...
      <section aria-labelledby="finance-kpis">
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-4">
          <StatTile
            ...
            value={
              transactionTotals
                ? formatNumber.format(transactionTotals.count)
                : undefined
            }
            sublabel={rangeLabel}
            isLoading={transactionVolume.status === "loading"}
            error={
              transactionVolume.status === "error"
                ? transactionVolume.error
                : null
            }
          />
          <StatTile
            ...
            value={
              transactionTotals
                ? formatCurrency.format(transactionTotals.volume)
                : undefined
            }
          ...
          <StatTile
            ...
            error={
              budgetAdherence.status === "error" ? budgetAdherence.error : null
            }
          />
          <StatTile
            icon={TrendingUp}
          ...
          <StatTile
            ...
            value={
              subscriptionsLoans.status === "success"
                ? formatInteger.format(
                    subscriptionsLoans.data.activeSubscriptions,
                  )
                : undefined
            }
            sublabel={
            ...
            error={
              subscriptionsLoans.status === "error"
                ? subscriptionsLoans.error
                : null
            }
          />
          <StatTile
          ...
          <StatTile
            ...
            value={
              typeof accountTotal === "number"
                ? formatInteger.format(accountTotal)
                : undefined
            }
          ...
          <StatTile
            ...
            isLoading={
              accountTypes.status === "loading" || overview.status === "loading"
            }
            error={
